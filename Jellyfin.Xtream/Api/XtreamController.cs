...
#pragma warning restore CA3003
    ...
    /// Tails the file and keeps sending data as ffmpeg writes it.
    /// Anonymous access allowed — the timer ID acts as an unguessable access token.
    /// </summary>
    /// <param name="timerId">The timer ID of the recording.</param>
...
#pragma warning restore CA3003
        ...
        try
        {
            ...
            while (!cancellation.IsCancellationRequested)
            {
                ...
                {
                    // No new data yet but recording is still in progress — wait briefly
                    await Task.Delay(250, cancellation).ConfigureAwait(false);
                }
                else
                {
                    // Recording has finished — drain all remaining bytes
                    while (!cancellation.IsCancellationRequested)
                    {
