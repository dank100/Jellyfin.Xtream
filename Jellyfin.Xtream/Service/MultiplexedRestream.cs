...
public class MultiplexedRestream : ILiveStream, IDirectStreamProvider, IDisposable
{
    ...
    /// <summary>Gets the number of gap-fill events.</summary>
    public long GapFillCount => Interlocked.Read(ref _gapFillCount);

    /// <summary>Gets the total bytes written by gap-fill null packets.</summary>
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                if (!wroteAny)
                {
                    ...
                    if (segmentsWritten > 0 && msSinceWrite >= gapFillThresholdMs)
                    {
                        ...
                        if (!hasFresh)
                        {
                            // Gap-fill: null TS packets (PID 0x1FFF) keep the TCP
                            // connection alive during capture gaps. No segment replay
                            // — replays cause visible content loops for viewers.
